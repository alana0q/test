...
import com.tentel.shared.terminal.io.BellHandler;
import com.tentel.shared.logger.Logger;
import com.tentel.terminal.TerminalColors;
import com.tentel.terminal.TerminalSession;
import com.tentel.terminal.TextStyle;
...
public class tentelTerminalSessionClient extends tentelTerminalSessionClientBase {

    private final tentelActivity mActivity;
    ...
    private static final String LOG_TAG = "tentelTerminalSessionClient";

    public tentelTerminalSessionClient(tentelActivity activity) {
    ...
    public void onStart() {
        ...
        if (mActivity.gettentelService() != null) {
            setCurrentSession(getCurrentStoredSessionOrLast());
            tentelSessionListNotifyUpdated();
        }
    ...
    public void onSessionFinished(final TerminalSession finishedSession) {
        tentelService service = mActivity.gettentelService();
        ...
        tentelSession tentelSession = service.gettentelSession(index);
        if (tentelSession != null) {
            isPluginExecutionCommandWithPendingResult = tentelSession.getExecutionCommand().isPluginExecutionCommandWithPendingResult();
        ...
        if (mActivity.getPackageManager().hasSystemFeature(PackageManager.FEATURE_LEANBACK)) {
            ...
            if (service.gettentelSessionsSize() > 1 || isPluginExecutionCommandWithPendingResult) {
    ...
    public void onBell(TerminalSession session) {
        ...
        switch (mActivity.getProperties().getBellBehaviour()) {
            case tentelPropertyConstants.IVALUE_BELL_BEHAVIOUR_VIBRATE:
                BellHandler.getInstance(mActivity).doBell();
                break;
            case tentelPropertyConstants.IVALUE_BELL_BEHAVIOUR_BEEP:
                getBellSoundPool().play(mBellSoundId, 1.f, 1.f, 1, 0, 1.f);
                break;
            case tentelPropertyConstants.IVALUE_BELL_BEHAVIOUR_IGNORE:
    ...
    public void switchToSession(boolean forward) {
        ...
        int size = service.gettentelSessionsSize();
        if (forward) {
        ...
        tentelSession tentelSession = service.gettentelSession(index);
        if (tentelSession != null)
            setCurrentSession(tentelSession.getTerminalSession());
    }
    ...
    public void addNewSession(boolean isFailSafe, String sessionName) {
        ...
        if (service.gettentelSessionsSize() >= MAX_SESSIONS) {
            ...
            tentelSession newtentelSession = service.createtentelSession(null, null, null, workingDirectory, isFailSafe, sessionName);
            if (newtentelSession == null) return;

            TerminalSession newTerminalSession = newtentelSession.getTerminalSession();
    ...
    public TerminalSession getCurrentStoredSessionOrLast() {
        ...
        if (stored != null) {
            ...
            tentelSession tentelSession = service.getLasttentelSession();
            if (tentelSession != null)
                return tentelSession.getTerminalSession();
            else
                return null;
        }
    }
    ...
    public void removeFinishedSession(TerminalSession finishedSession) {
        ...
        int index = service.removetentelSession(finishedSession);
    ...
    public void tentelSessionListNotifyUpdated() {
        mActivity.tentelSessionListNotifyUpdated();
    }
    ...
    public void checkAndScrollToSession(TerminalSession session) {
        ...
        final ListView tentelSessionsListView = mActivity.findViewById(R.id.terminal_sessions_list);
        if (tentelSessionsListView == null) return;

        tentelSessionsListView.setItemChecked(indexOfSession, true);
        // Delay is necessary otherwise sometimes scroll to newly added session does not happen
        tentelSessionsListView.postDelayed(() -> tentelSessionsListView.smoothScrollToPosition(indexOfSession), 1000);
    }
    ...
    public void checkForFontAndColors() {
        try {
            File colorsFile = tentelConstants.tentel_COLOR_PROPERTIES_FILE;
            File fontFile = tentelConstants.tentel_FONT_FILE;
