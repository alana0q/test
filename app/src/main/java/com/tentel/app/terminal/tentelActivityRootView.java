...
/**
 * The {@link tentelActivity} relies on {@link android.view.WindowManager.LayoutParams#SOFT_INPUT_ADJUST_RESIZE)}
 * set by {@link tentelTerminalViewClient#setSoftKeyboardState(boolean, boolean)} to automatically
 * resize the view and push the terminal up when soft keyboard is opened. However, this does not
 * always work properly. When `enforce-char-based-input=true` is set in `tentel.properties`
 * and {@link com.tentel.view.TerminalView#onCreateInputConnection(EditorInfo)} sets the inputType
 * to `InputType.TYPE_TEXT_VARIATION_VISIBLE_PASSWORD | InputType.TYPE_TEXT_FLAG_NO_SUGGESTIONS`
 * instead of the default `InputType.TYPE_NULL` for tentel, some keyboards may still show suggestions.
 ...
 * Also check https://github.com/tentel/tentel-app/issues/1539.
 *
 * This overlap may happen even without `enforce-char-based-input=true` for keyboards with extended layouts
 * like number row, etc.
 *
 * To fix these issues, `activity_tentel.xml` has the constant 1sp transparent
 * `activity_tentel_bottom_space_view` View at the bottom. This will appear as a line matching the
 * activity theme. When {@link tentelActivity} {@link ViewTreeObserver.OnGlobalLayoutListener} is
 ...
 */
public class tentelActivityRootView extends LinearLayout implements ViewTreeObserver.OnGlobalLayoutListener {

    public tentelActivity mActivity;
    ...
    private static final String LOG_TAG = "tentelActivityRootView";

    private static int mStatusBarHeight;

    public tentelActivityRootView(Context context) {
        super(context);
    }

    public tentelActivityRootView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }

    public tentelActivityRootView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    public void setActivity(tentelActivity activity) {
        mActivity = activity;
    }
    ...
    public void onGlobalLayout() {
        ...
        View bottomSpaceView = mActivity.gettentelActivityBottomSpaceView();
        ...
        if (isVisible) {
            ...
            // Calling addtentelActivityRootViewGlobalLayoutListener with a delay fails to
