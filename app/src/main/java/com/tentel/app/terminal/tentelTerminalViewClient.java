package com.tentel.app.terminal;
...
import com.tentel.shared.data.UrlUtils;
...
import com.tentel.shared.terminal.tentelTerminalViewClientBase;
import com.tentel.shared.terminal.io.extrakeys.SpecialButton;
...
import com.tentel.shared.view.KeyboardUtils;
import com.tentel.shared.view.ViewUtils;
import com.tentel.terminal.KeyHandler;
import com.tentel.terminal.TerminalBuffer;
...
public class tentelTerminalViewClient extends tentelTerminalViewClientBase {

    final tentelActivity mActivity;

    final tentelTerminalSessionClient mtentelTerminalSessionClient;
    ...
    private static final String LOG_TAG = "tentelTerminalViewClient";

    public tentelTerminalViewClient(tentelActivity activity, tentelTerminalSessionClient tentelTerminalSessionClient) {
        this.mActivity = activity;
        this.mtentelTerminalSessionClient = tentelTerminalSessionClient;
    }

    public tentelActivity getActivity() {
        return mActivity;
    }
    ...
    public void onStart() {
        ...
        // Also required if user changed the preference from {@link tentelSettings} activity and returns
        ...
        mActivity.gettentelActivityRootView().setIsRootViewLoggingEnabled(isTerminalViewKeyLoggingEnabled);
    ...
    public void onResume() {
        ...
        if (mActivity.getTerminalView().mEmulator != null) {
            ...
            // tentelActivity is started after device display timeout with double tap and not power button.
    ...
    /**
     * Should be called when {@link com.tentel.view.TerminalView#mEmulator} is set
     */
    @Override
    public void onEmulatorSet() {
        if (!mTerminalCursorBlinkerStateAlreadySet) {
            ...
            // tentelActivity.onServiceConnected() and then the multiple calls to TerminalView.updateSize()
            // where the final one eventually sets the mEmulator when width/height is not 0. Otherwise
            // blinker will not start again if tentelActivity is started again after exiting it with
    ...
    public boolean onKeyDown(int keyCode, KeyEvent e, TerminalSession currentSession) {
        ...
        if (keyCode == KeyEvent.KEYCODE_ENTER && !currentSession.isRunning()) {
            mtentelTerminalSessionClient.removeFinishedSession(currentSession);
            return true;
            ...
            if (keyCode == KeyEvent.KEYCODE_DPAD_DOWN || unicodeChar == 'n'/* next */) {
                mtentelTerminalSessionClient.switchToSession(true);
            } else if (keyCode == KeyEvent.KEYCODE_DPAD_UP || unicodeChar == 'p' /* previous */) {
                mtentelTerminalSessionClient.switchToSession(false);
                ...
                mtentelTerminalSessionClient.renameSession(currentSession);
            } else if (unicodeChar == 'c'/* create */) {
                mtentelTerminalSessionClient.addNewSession(false, null);
                ...
                // see https://github.com/tentel/tentel-api/issues/2
                changeFontSize(true);
            } else if (unicodeChar == '-') {
                changeFontSize(false);
            } else if (unicodeChar >= '1' && unicodeChar <= '9') {
                int index = unicodeChar - '1';
                mtentelTerminalSessionClient.switchToSession(index);
            }
            return true;
        }
        ...
        if (mVirtualFnKeyDown) {
            ...
            switch (lowerCase) {
                ...
                case 'k':
                    ...
                    mVirtualFnKeyDown=false; // force disable fn key down to restore keyboard input into terminal view, fixes tentel/tentel-app#1420
                    break;
            }
            ...
            if (codePoint == 106 /* Ctrl+j or \n */ && !session.isRunning()) {
                mtentelTerminalSessionClient.removeFinishedSession(session);
                return true;
            }
                ...
                for (int i = shortcuts.size() - 1; i >= 0; i--) {
                    ...
                    if (codePointLowerCase == shortcut.codePoint) {
                        switch (shortcut.shortcutAction) {
                            case tentelPropertyConstants.ACTION_SHORTCUT_CREATE_SESSION:
                                mtentelTerminalSessionClient.addNewSession(false, null);
                                return true;
                            case tentelPropertyConstants.ACTION_SHORTCUT_NEXT_SESSION:
                                mtentelTerminalSessionClient.switchToSession(true);
                                return true;
                            case tentelPropertyConstants.ACTION_SHORTCUT_PREVIOUS_SESSION:
                                mtentelTerminalSessionClient.switchToSession(false);
                                return true;
                            case tentelPropertyConstants.ACTION_SHORTCUT_RENAME_SESSION:
                                mtentelTerminalSessionClient.renameSession(mActivity.getCurrentSession());
                                return true;
                        }
                    }
                }
    ...
    public void onToggleSoftKeyboardRequest() {
        ...
        // If soft keyboard toggle behaviour is show/hide
            ...
            // If soft keyboard is disabled by user for tentel
    ...
    public void setSoftKeyboardState(boolean isStartup, boolean isReloadtentelProperties) {
        ...
        // If soft keyboard is disabled by user for tentel (check function docs for tentel behaviour info)
        if (KeyboardUtils.shouldSoftKeyboardBeDisabled(mActivity,
            ...
            mActivity.getTerminalView().requestFocus();
            noShowKeyboard = true;
            // Delay is only required if onCreate() is called like when tentel app is exited with
            // double back press, not when tentel app is switched back from another app and keyboard
            ...
            if (isStartup && mActivity.getProperties().shouldSoftKeyboardBeHiddenOnStartup()) {
                ...
                // Required to keep keyboard hidden when tentel app is switched back from another app
        ...
        // Do not force show soft keyboard if tentel-reload-settings command was run with hardware keyboard
        // or soft keyboard is to be hidden or is disabled
        if (!isReloadtentelProperties && !noShowKeyboard) {
            ...
            // "Select URL" long press and returning to tentel app with back button. This
    ...
    public void shareSessionTranscript() {
        ...
        try {
            // See https://github.com/tentel/tentel-app/issues/1166.
    ...
    public void reportIssueFromTranscript() {
        ...
        MessageDialogUtils.showMessage(mActivity, tentelConstants.tentel_APP_NAME + " Report Issue",
            mActivity.getString(R.string.msg_add_tentel_debug_info),
    ...
    private void reportIssueFromTranscript(String transcriptText, boolean addtentelDebugInfo) {
        ...
        new Thread() {
            ...
            public void run() {
                ...
                String title = tentelConstants.tentel_APP_NAME + " Report Issue";

                reportString.append("## Transcript\n");
                reportString.append("\n").append(MarkdownUtils.getMarkdownCodeForString(transcriptText, true));
                reportString.append("\n##\n");

                reportString.append("\n\n").append(tentelUtils.getAppInfoMarkdownString(mActivity, true));
                reportString.append("\n\n").append(AndroidUtils.getDeviceInfoMarkdownString(mActivity));

                String tentelAptInfo = tentelUtils.geAPTInfoMarkdownString(mActivity);
                if (tentelAptInfo != null)
                    reportString.append("\n\n").append(tentelAptInfo);

                if (addtentelDebugInfo) {
                    String tentelDebugInfo = tentelUtils.gettentelDebugMarkdownString(mActivity);
                    if (tentelDebugInfo != null)
                        reportString.append("\n\n").append(tentelDebugInfo);
                }
                ...
                ReportActivity.startReportActivity(mActivity,
                    new ReportInfo(userActionName,
                        tentelConstants.tentel_APP.tentel_ACTIVITY_NAME, title, null,
                        reportString.toString(), "\n\n" + tentelUtils.getReportIssueMarkdownString(mActivity),
                        false,
                        userActionName,
