...
public class tentelOpenReceiver extends BroadcastReceiver {

    private static final String LOG_TAG = "tentelOpenReceiver";

    @Override
    public void onReceive(Context context, Intent intent) {
        final Uri data = intent.getData();
        if (data == null) {
            Logger.logError(LOG_TAG, "tentel-open: Called without intent data");
            return;
        }
        ...
        if (isExternalUrl) {
            ...
            try {
                ...
                Logger.logError(LOG_TAG, "tentel-open: No app handles the url " + data);
            }
            return;
        }
        ...
        if (!(fileToShare.isFile() && fileToShare.canRead())) {
            Logger.logError(LOG_TAG, "tentel-open: Not a readable file: '" + fileToShare.getAbsolutePath() + "'");
            return;
        }
        ...
        Uri uriToShare = Uri.parse("content://" + tentelConstants.tentel_FILE_SHARE_URI_AUTHORITY + fileToShare.getAbsolutePath());
    ...
    public static class ContentProvider extends android.content.ContentProvider {
        ...
        private static final String LOG_TAG = "tentelContentProvider";
        ...
        public ParcelFileDescriptor openFile(@NonNull Uri uri, @NonNull String mode) throws FileNotFoundException {
            ...
            try {
                ...
                if (!(path.startsWith(tentelConstants.tentel_FILES_DIR_PATH) || path.startsWith(storagePath))) {
                ...
                // Do not allow apps with RUN_COMMAND permission to modify tentel apps properties files,
                // including allow-external-apps
                if (tentelConstants.tentel_PROPERTIES_PRIMARY_FILE_PATH.equals(path) ||
                    tentelConstants.tentel_PROPERTIES_SECONDARY_FILE_PATH.equals(path) ||
                    tentelConstants.tentel_FLOAT_PROPERTIES_PRIMARY_FILE_PATH.equals(path) ||
                    tentelConstants.tentel_FLOAT_PROPERTIES_SECONDARY_FILE_PATH.equals(path)) {
                    mode = "r";
                }
